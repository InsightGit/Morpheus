...
hayai::Player::Player(std::shared_ptr<morpheus::core::MainLoop> main_loop,
    ...
    level_background->set_scroll(morpheus::core::gfx::Vector2(30*8, 30*8));
        ...
        switch (input_event.button) {
            ...
            default:
                if(!m_moved_this_frame) {
                    m_acceleration = morpheus::core::gfx::Vector2(m_acceleration.get_x() - ACCELERATION_STEP,
                                                                  m_acceleration.get_y() - ACCELERATION_STEP);
                }
                break;
        }
    }
...
void hayai::Player::restrict_velocity(bool x, bool pos) {
    if(m_current_level != nullptr && m_level_background != nullptr) {
        std::vector<unsigned int> collision_tile_ids = m_current_level->get_collision_tile_ids();
        ...
        for(int i = 0; static_cast<int>(tile_ids_to_check.size()) > i; ++i) {
            ...
            for(int i2 = 0; static_cast<int>(collision_tile_ids.size()) > i; ++i2) {
                if(collision_tile_ids[i2] == tile_ids_to_check[i]) {
                    nocash_puts(std::string("tile id " + std::to_string(tile_ids_to_check[i]) + "matches").c_str());

                    if(x) {
                        if(pos) {
                            m_velocity = morpheus::core::gfx::Vector2(std::min(8 * (i-1), m_velocity.get_x()),
                                                                      m_velocity.get_y());
                        } else {
                            m_velocity = morpheus::core::gfx::Vector2(std::max(-8 * (i-1), m_velocity.get_x()),
                                                                      m_velocity.get_y());
                        }
                    } else {
                        if(pos) {
                            m_velocity = morpheus::core::gfx::Vector2(m_velocity.get_x(),
